 ...
 * 作者姓名     修改时间     版本号        描述
 */
public class SurveyResultProcessNewClassicUpdate {
     ...
     * @param st st
     */
    private static void calcStXYZ0(SurveyStation st){
        double ee = 0.01;
        //获取终点坐标初始值已知的测线集合、测点集合、只包括测站点和已知点的测站对象
        List<SurveyLine> lines = st.getSurveyLines().stream().filter(line -> Math.abs(line.getPoint2().getX0() - 9999999999.0) > ee).collect(Collectors.toList());
        SurveyStation newSt = new SurveyStation();
        newSt.setStation(st.getStation());
        newSt.setSurveyLines(lines);

        List<Point3d> pts = new ArrayList<>();
        pts.add(st.getStation());
        for (SurveyLine line : lines) {
            pts.add(line.getPoint2());
        }

        //获取3中计算方法获得的测站近似坐标结果列表
        List<StXyz0> st0List = new ArrayList<>();
        for (int i = 0; i < lines.size() - 1; i++) {
            SurveyLine l1 = lines.get(i);
            for (int j = i+1; j < lines.size(); j++) {
                SurveyLine l2 = lines.get(j);
                StXyz0 st0 = new StXyz0();
                resectionFront1(l1,l2,st.getHi(), st0);
                st0List.add(st0);

                st0 = new StXyz0();
                resectionFront2(l1,l2,st.getHi(), st0);
                st0List.add(st0);

                st0 = new StXyz0();
                resectionDist(l1,l2,st.getHi(), st0);
                st0List.add(st0);
            }
        }

        for (StXyz0 stXyz0 : st0List) {
            double vp = 0.0;
            st.getStation().setX0(stXyz0.getX0());
            st.getStation().setY0(stXyz0.getY0());
            st.getStation().setZ0(stXyz0.getZ0());

            vp = adjustForSt0(pts, newSt);
            stXyz0.setVp(vp);
        }

        //按vp升序排列，取最小值对应的坐标作为测站最优概略坐标
        st0List = st0List.stream().sorted(Comparator.comparingDouble(StXyz0::getVp)).collect(Collectors.toList());
        st.getStation().setX0(st0List.get(0).getX0());
        st.getStation().setY0(st0List.get(0).getY0());
        st.getStation().setZ0(st0List.get(0).getZ0());
    }

    @Setter
    @Getter
    private static class StXyz0{
        private double X0;
        private double y0;
        private double z0;
        private double vp;//测站概略坐标平差后改正数偏差
    }

    /**
     * 取顶点夹角+S2计算概略坐标
     */
    private static void resectionFront1(SurveyLine l1, SurveyLine l2, double hi, StXyz0 st) {
        //夹角1-St-2
        ...
        //st->1连线为l1，平距为s1,所对应夹角为b1;st->2连线为l2,平距为s2，对应夹角为b2;1->2连线平距为S0，对应夹角为deltHa0
        int fh = deltHa0 < Math.PI ? -1 : 1;
        //超过PI的夹角，转为反向内角
        deltHa0=deltHa0>Math.PI? 2* Math.PI-deltHa0 :deltHa0;
        ...
        double b1 = Math.asin(s2 / s0 * Math.sin(deltHa0));
        double b2 = Math.PI - deltHa0 - b1;
        double cotb1 = 1.0 / Math.tan(b1);
        double cotb2 = 1.0 / Math.tan(b2);

        st.setX0((x2*cotb1+x1*cotb2+fh *deltY)/(cotb1+cotb2));
        st.setY0((y2*cotb1+y1*cotb2-fh *deltX)/(cotb1+cotb2));
        st.setZ0(l2.getPoint2().getZ0() - l2.getSd0() * Math.cos(l2.getVa0()) - (hi - l2.getHt()));
    }

    /**
     * 取顶点夹角+S1计算概略坐标
     */
    private static void resectionFront2(SurveyLine l1, SurveyLine l2, double hi, StXyz0 st) {
        //夹角1-St-2
        double deltHa0 = l2.getHa0() - l1.getHa0();
        //delHa0若为负值，转正值
        if (deltHa0 < 0) {
        ...
        double b2 = Math.asin(s1 / s0 * Math.sin(deltHa0));
        double b1 = Math.PI - deltHa0 - b2;
        double cotb1 = 1.0 / Math.tan(b1);
        double cotb2 = 1.0 / Math.tan(b2);

        st.setX0((x2*cotb1+x1*cotb2+fh *deltY)/(cotb1+cotb2));
        st.setY0((y2*cotb1+y1*cotb2-fh *deltX)/(cotb1+cotb2));
        st.setZ0(l1.getPoint2().getZ0() - l1.getSd0() * Math.cos(l1.getVa0()) - (hi - l1.getHt()));
    }

    /**
     * 距离交会计算概略坐标
     */
    private static void resectionDist(SurveyLine l1, SurveyLine l2, double hi, StXyz0 st) {
        double x1 = l1.getPoint2().getX0();
        double y1 = l1.getPoint2().getY0();
        double x2 = l2.getPoint2().getX0();
        double y2 = l2.getPoint2().getY0();
     ...
     * 用于测站近似坐标计算方法比对的平差算法
     */
    private static double adjustForSt0(List<Point3d> ptList,SurveyStation station) {
        List<SurveyStation> stations = new ArrayList<>();
        stations.add(station);

        boolean isOk = ptList.size() > 2;
        if (!isOk) return 0.0;
        ...
        //未知点数
        int u = 0;
        List<Point3d> uPtList = ptList.stream().filter(it -> !it.isAsFixed()).collect(Collectors.toList());
        u = uPtList.size();
        m =  u*3 + stations.size() + 1;
