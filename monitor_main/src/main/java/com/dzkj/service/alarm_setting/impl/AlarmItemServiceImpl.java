package com.dzkj.service.alarm_setting.impl;

import com.baomidou.mybatisplus.extension.service.impl.ServiceImpl;
import com.dzkj.entity.alarm_setting.AlarmItem;
import com.dzkj.mapper.alarm_setting.AlarmItemMapper;
import com.dzkj.service.alarm_setting.IAlarmItemService;
import org.springframework.stereotype.Service;

import java.util.List;

/**
 * Copyright(c),2018-2021,合肥市鼎足空间技术有限公司
 *
 * @author jing.fang
 * @date 2021/8/26
 * @description 报警项服务实现
 * history
 * <author>    <time>    <version>    <desc>
 *  作者姓名     修改时间     版本号        描述
 */
@Service
public class AlarmItemServiceImpl extends ServiceImpl<AlarmItemMapper, AlarmItem> implements IAlarmItemService {

    @Override
    public List<AlarmItem> getByPid(Long pid) {
        return baseMapper.findByPid(pid);
    }

    @Override
    public List<AlarmItem> getByPidList(List<Long> pidList) {
        return baseMapper.getByPidList(pidList);
    }
}
